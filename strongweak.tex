...
\begin{remark}\label{rem_H}
  ...
  Therefore for every $f\in G$ there is a ${\gr g}\in\grZ$ such that $f[\mrD]={\gr g}{\cdot}\mrD$ holds for every $\LDelta({\gr B})$-definable set $\mrD$.
  We conclude that for $\LDelta({\gr B})$-definable sets the notions of generic and persistent under the actions of $G$ and $\grZ$ coincide.
\end{remark}
...
\begin{question}
  Can we replace the group $\big\<\bigcup_{{\gr g}\in\grZ}({\gr g})_H^{-1}\!\cdot({\gr g})_H\big\>$ in Proposition~\ref{prop_stabilizer1} with an $H$-invariant group?
\end{question}
