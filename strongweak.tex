...
A $\Delta$-definable set is a set of the form $\theta(\U^{\mr x})$ for some $\Delta$-formula $\theta({\mr x})\in B_\Delta({\grZ})$.
Subsets of $B_\Delta({\grZ})$ are called $\Delta$-types.
We write $S_\Delta({\grZ})$ for the set of complete $\Delta$-types with parameters in $\grZ$.
Note that complete $\Delta$-types are equivalent to subsets of $L_{\pmDelta}({\grZ})$.
% Beware that there may be other parameters hidden in $\Delta$.
...
\begin{corollary}\label{corol_def_mu}
  ...
  \begin{itemize}
    ...
    \item[2.] if $\mrD$ is generic then it is persistent
    \item[3.] the type $\gamma_G({\mr x})=\{\theta({\mr x})\in B_\Delta({\grZ}): \theta({\mr x})\textrm{ generic }\}$ is finitely satisfiable in $\mrX$.
  \end{itemize}
\end{corollary}

\begin{proof}
  The three claims are equivalent; \ssf1 is an immediate consequenc of \ssf2 of Theorem~\ref{thm_generic_invariant}.
\end{proof}
...
A set $\mrD\subseteq\U^{\mr x}$ is \emph{strongly generic\/} if for every finite $H\subseteq G$ the set $\cap\,H\,\mrD$ is generic.
Dually, we say that $\mrD$ is \emph{weakly persistent\/} if for some finite $H\subseteq G$ the set $\cup\,H\,\mrD$ is persistent.
...
\begin{proof}
  ...
  As ${\mr\C'}$ is strongly generic, $\cap\, H\,{\mr\C'}$ is generic.
...
\begin{corollary}\label{corol_str_gen}
  ...
  \ceq{\hfill{}^{\rm s}\kern-.2ex\gamma_G({\mr x})}{=}{\big\{\theta({\mr x})\in B_\Delta({\grZ})\,:\, \theta({\mr x})\textrm{ strongly generic}\big\}.}
  
  Then ${}^{\rm s}\kern-.2ex\gamma_G({\mr x})$ is finitely satisfiable in $\mrX$, strongly generic, and invariant.
\end{corollary}
...
\begin{corollary}\label{corol_q_w_pers}
  (Assume~\ref{notation_GXphi})\ \  
  Let ${}^{\rm s}\kern-.2ex\gamma_G({\mr x})$ be as in Corollary~\ref{corol_str_gen}.
  Let $p({\mr x})\subseteq B_\Delta({\grZ})$ be such that $p({\mr x})\cup{}^{\rm s}\kern-.2ex\gamma_G({\mr x})$ is finitely satisfied in $\mrX$.
  Then $p({\mr x})$ is weakly persistent.
\end{corollary}

\begin{proof}
  Let $\theta({\mr x})\in p$.
  As ${}^{\rm s}\kern-.2ex\gamma_G({\mr x})$ is finitely satisfiable in $\theta(\U^{\mr x})$, we cannot have that $\neg\theta({\mr x})$ is strongly generic.
...
\begin{example}
  ...
  Then the group $G$ generated by $K$ is $\Autf(\U/A)$ and $G\,{\mr a}=\mrX$ is $\Ll({\mr a}/A)$.
  Then $d({\mr a},{\mr b})$ concides with the dinstance in the Lascar graph.
  It is not difficult to see that the sets $K^n\,{\mr a}$ are type definable.
...
\begin{proof}
  ...
  \ssf2$\IFF$\ssf3$\IFF$\ssf4.
  Clear.

  \ssf4$\IMP$\ssf1.
  Let ${}^{\rm s}\kern-.2ex\gamma_G({\mr x})$ be as in Corollary~\ref{corol_q_w_pers}.
  Any completion of ${}^{\rm s}\kern-.2ex\gamma_G({\mr x})$ is, by \ssf4, persistent.
  Let $\mrD$ be a persistent $\Delta$-definable set.
  By Theorems~\ref{thm_generic_invariant} and~\ref{thm_generic_invariant2} it suffices to show that $\mrD$ is consistent with ${}^{\rm s}\kern-.2ex\gamma_G({\mr x})$.
  Suppose not, then ${}^{\rm s}\kern-.2ex\gamma_G({\mr x})\proves {\mr x}\notin\mrD$.
  Therefore $\neg\mrD$ is generic.
...
\begin{remark}\label{rem_coalesce}
  ...
  \end{itemize}
  Note also that, as $\gamma_G({\mr x})$ is finitely consistent on $\mrX$, invariant global types exist.

  It is also worth mentioning that if $\mrD$ is generic then every positive Bolean combination of $G$-translates of $\mrD$ is generic.
\end{remark}
