...
\begin{abstract}
  \setlength{\parindent}{0ex}
  \setlength{\parskip}{.4\baselineskip}
  In a nutshell, a set is \textit{strongly generic\/} if the intersection of any finitely many of its translation is generic. 
  I do not know if the notion has been studied in the context presented here. To demostrate its interest we use it for a short proof of (a generalization of) Newelski's theorem on the diamter of the Lascar graph, see Theorem~\ref{thm_newelski}.
  
  Theorem~\ref{thm_coalesce} shows that the phenomenon \textit{strongly generic = generic\/} has some interest (it is reminiscent of \textit{forking = dividing\/}).
  ...
  I think I am missing something important (or commit an error somewhere).
\end{abstract}
...
This section review well-known matter and set some terminology.
...
We write $S_\Delta({\grZ})$ for the set of complete $\Delta$-types with parameters in $\grZ$ (there may be other parameters hidden ion $\Delta)$.
...
In~\cite{CK} a related property is called \textit{quasi-non-forking,} but we persist in the unorthodoxy.
...
The group operations and the group action are required to be definable over $A$.
...
Generic and persistent only refer to the action of $\grZ$, we will always be explicit about invariance.
...
The types $p({\mr x})\in S_\Delta(\grZ)$ that extend $q({\mr x})$ are persistent, hence invariant under the action of both $\grZ$ and $\Aut(\U/A)$.
