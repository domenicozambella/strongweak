...
When convenient, we may asume that complete types only contain formulas of the form $\pm\phi({\mr x}\,;{\gr b})$ for $\phi({\mr x}\,;{\gr z})\in\Delta$.
...
In this section we work as always under Assumption~\ref{notation_GXphi} but with add som,e important requirement on $G$ and $\Delta$.
...
An immediate consequence is that $\Aut(\U/A)$ maps generic/persistent sets of the form $\pm\phi(\mrX\,;{\gr g}\,;a)$ to generic/persistent sets of the same form.
...
\begin{proof}
  We may assume that $p({\mr x})$ only contains formulas of the form $\pm\phi({\mr x}\,;{\gr b})$ for $\phi({\mr x}\,;{\gr z})\in\Delta$.
