...
A set $\mrD\subseteq\mrX$ is \emph{syndetic\/} under the action of $G$, or \emph{$G$-syndetic\/} for short, if finitely many $G$-translates of $\mrD$ cover $\mrX$; we say \emph{$n$-$G$-syndetic\/} if $\le n$ translates suffices.
Dually, we say that $\mrD$ is \emph{thick\/} under the action of $G$, or \emph{$G$-thick\/} for short, if the intersection of any finitely many $G$-translates of $\mrD$ is nonempty; we say \emph{$n$-$G$-thick\/} when the request is limited to $\le n$ translates.
...
\section{Strong syndeticty}\label{strong_syndeticty}
...
A set $\mrD\subseteq\U^{\mr x}$ is \emph{strongly syndetic\/} if the intersection of $\mrD$ with any of its tranlations $\mrD$ is syndetic.
Dually, we say that $\mrD$ is \emph{weakly thick\/} if the union of $\mrD$ with one of its translations is peristent.
...
\begin{lemma}\label{lem_strongly_generic}
  (Assume~\ref{notation_GXphi})\ \  
  The intersection of strongly syndetic sets is strongly syndetic.
\end{lemma}

\begin{proof}
  We may assume that all sets mentioned below are subsets of $\mrX$.
  Let ${\mr\D}$ and ${\mr\C}$ be strongly syndetic and let $K\subseteq G$ be an arbitrary finite set.
  It suffices to prove that $\mrB=\cap\, K\,({\mr\C}\cap{\mr\D})$ is syndetic. 
  Clearly $\mrB={\mr\C'}\cap{\mr\D'}$, where ${\mr\C'}=\cap\, K\,{\mr\C}$ and ${\mr\D'}=\cap\, K\,{\mr\D}$.
  Note that ${\mr\C'}$ and ${\mr\D'}$ are both strongly syndetic.
  ...
  As ${\mrC'}$ is strongly syndetic, $\cap\, H\,{\mr\C'}$ is syndetic.
  Therefore $\cup\,H\,\mrB$ is also syndetic.
  The syndeticty of $\mrB$ follows.
\end{proof}
...
The notions introduced in Section~\ref{strong_syndeticty} offer some hints.

\begin{proposition}\label{prop_wpers_finite_diameter}
  If $\mrX$ has a weakly thick subset of finite diameter, then $\mrX$ itself has finite diamenter.
\end{proposition}

\begin{proof}
  Let $\mrC\subseteq\mrX$ be a weakly thick set of diameter $n$.
  Let $H\subseteq G$ be finite such that $\cup\,H\,\mrC$ is thick.
...
\begin{proof}
  ...
  \ceq{\hfill q({\mr x})}{=}{\{\theta({\mr x})\in L(\U)\ :\ \theta({\mr x})\textrm{ strongly syndetic}\}.}
...
\begin{example}
  Let $G_1\subseteq\Aut(\U/A)$ be the set of automorphisms that fix a model containing $A$.
  Thenthe group $G$  generated by $G_1$ is $\Autf(\U/A)$ and $G\cdot{\mr a}=\mrX$ is $\Ll({\mr a}/A)$.
  ...
  It is not difficult to see that the sets $G_n\cdot{\mr a}$ are type definable.
...
Syndetic and thick only refer to the action of $\grZ$.
We will always be explicit about invariance.
...
Note that $\phi(\mrX\,;\grZ\,;a)$ is invariant under the action of $\grZ$
and $\phi(\mrX\,;{\gr g}\,;a)={\gr g}\cdot\phi(\mrX\,;{\gr 1}\,;a)$ for every $a$.
Therefore the above $G$ and $\Delta$ satisfy Assumption~\ref{notation_GXphi}.
...
\begin{fact}
  ...
  \ceq{\hfill\gamma(a)}{\IFF}{\phi({\mr x}\,;{\gr 1}\,;a)\textrm{ is syndetic}}

  \ceq{\hfill\pi(a)}{\IFF}{\phi({\mr x}\,;{\gr 1}\,;a)\textrm{ is thick}.}

  Similar claims holds for $\neg\phi({\mr x}\,;{\gr 1}\,;a)$.
\end{fact}

\begin{proof}
  By an easy argument of compactness, there is an $n$ such that if $\phi({\mr x}\,;{\gr 1}\,;a)$ is syndetic then it is also $n$-syndetic.
  Then
...
As $\phi({\mr x}\,;{\gr g}\,;a)$ is syndetic/thick if and only if $\phi({\mr x}\,;{\gr 1}\,;a)$ is such, $\Aut(\U/A)$ maps syndetic/thick sets of the form $\phi(\mrX\,;{\gr g}\,;a)$ to syndetic/thick sets of the same form.
% The same is true for syndetic/thick types $p({\mr x})\in S_\Delta(\grZ)$ in fact, up to equivalence, we may assume that they contain only formulas of the form $\phi({\mr x}\,;{\gr g}\,;a)$ and negation thereof.
...
%   \begin{itemize}
%     \item[1.] $\phi({\mr x}\,;{\gr 1})$ is $\grG$-syndetic
%     \item[2.] $\mrD$ is syndetic over $A$.
%   \end{itemize}
% \end{fact}
