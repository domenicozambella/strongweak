...
\begin{abstract}
  ...
  Proposition~\ref{prop_stabilizer1} reminds of Hrushovski's stabilizer theorem~\cite{Hr}.
  But how, exactly? And, can it be strengthened to obtain a type definable group?
...
Under suitable assumptions -- e.g.\@ stability -- some  notion introduced in this chapter coalesce, and we are left with a tamer landscape.
...
\begin{proof}
  ...
  Equivalently, $g^{-1}p\proves{\mr x}\in\mrD$.
  As $p({\mr x})$ is $H$-persistent, by Remark~\ref{rem_invariance_normalsubg} $g^{-1}p({\mr x})$ is also $H$-persistent.
...
As $\mrX$ and $\grZ$ are assumed to be type-definable, $\UDelta$ is a saturated $\LDelta$-structure.
...
We conclude that for formulas in $\pmDelta(\grZ)$ the notions of generic and persistent under the two actions coincide.
...
\begin{proof}
  Let $f_i\in H$ be such that ${\gr h_i}\in f_i({\gr g_i})$.
  Without loss of generality we can assume that $\theta({\mr x}\,;{\gr g_1},\dots,{\gr g_n})$ is the conjunction of the formulas $\phi_i({\mr x}\,;{\gr g_i})$.
  ...
  Then \ssf1$\IMP$\ssf2 follows from Remark~\ref{rem_H}.
  By symmetry, this proves the equivalence.
\end{proof}

If $\grA\subseteq\grZ$, write \emph{$\<\grA\>$} for the subgroup generated by $\grA$.

\begin{proposition}\label{prop_stabilizer1}
  Let $\theta({\mr x}\,;{\gr z_1},\dots,{\gr z_n})$ be a Boolean combination of formulas $\phi_i({\mr x}\,;{\gr z_i})$ for some $\phi_i({\mr x}\,;{\gr z})\in\Delta$.
  Let ${\gr g}\in\grZ$ be arbitrary.
  Assume that $\theta({\mr x}\,;{\gr 1},\dots,{\gr 1})$ is $H$-wide.
  Then $\theta({\mr x}\,;{\gr h_1},\dots,{\gr h_n})$ is $H$-wide for every ${\gr h_i}\in\big\<\,({\gr g})_H^{-1}\!\cdot({\gr g})_H\ \cup\ ({\gr g})_H\cdot({\gr g})_H^{-1}\big\>$.
\end{proposition}

\begin{proof}
  We proceed by induction on the number of factors ${\gr a^{-1}}\!\cdot{\gr b}$ or ${\gr a}\cdot{\gr b^{-1}}$, for some ${\gr a},{\gr b}\in({\gr g})_H$, that occur in ${\gr h_1},\dots,{\gr h_n}$.
  Without loss of generality we can assume that $\theta({\mr x}\,;{\gr z_1},\dots,{\gr z_n})$ is the conjunction of the formulas $\phi_i({\mr x}\,;{\gr z_i})$ for some $\phi_i({\mr x}\,;{\gr z})\in\Delta$.
  Let ${\mr\C_i}=\phi_i(\mrX\,;{\gr 1})$.
  ...
  A similar argument applies to ${\gr a}\!\cdot{\gr b^{-1}}$.
\end{proof}
